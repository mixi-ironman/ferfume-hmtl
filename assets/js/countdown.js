function egaFlashSale() {
    ...
    function renderProgress(item) {
        ...
        if (useSoldQuantity) {
            ...
            percent = !flashSaleStorage[id].soldQty
                ? randomQty()
                : Math.round((flashSaleStorage[id].soldQty * 100) / stock);
            if (management) {
                soldQty = !flashSaleStorage[id].soldQty
                    ? Math.round((100 * parseInt(inventory)) / (100 - percent))
                    : flashSaleStorage[id].soldQty;
                stock = soldQty + parseInt(inventory);
            } else {
                soldQty = !flashSaleStorage[id].soldQty
                    ? Math.round((percent * stock) / 100)
                    : flashSaleStorage[id].soldQty;
            }
        }
        ...
        flashSaleStorage[id].timestamp = !flashSaleStorage[id].timestamp
            ? new Date().getTime()
            : flashSaleStorage[id].timestamp;
    ...
    function renderCountDown(distance) {
        ...
        let hours = `<div><div class="ega-badge-ctd__item ega-badge-ctd__h">${
            getHours(distance) >= 10 ? getHours(distance) : `0${getHours(distance)}`
        }</div><span>Giờ</span> </div>`;
        let minutes = `<div><div class="ega-badge-ctd__item  ega-badge-ctd__m">${
            getMinutes(distance) >= 10 ? getMinutes(distance) : `0${getMinutes(distance)}`
        }</div><span>Phút</span></div>`;
        let seconds = `<div><div class="ega-badge-ctd__item ega-badge-ctd__s">${
            getSeconds(distance) >= 10 ? getSeconds(distance) : `0${getSeconds(distance)}`
        }</div><span>Giây</span></div>`;
        html = [hours, minutes, seconds];
        if (days > 0)
            html.unshift(
                `<div><div class="ega-badge-ctd__item ega-badge-ctd__d">${
                    days >= 10 ? days : `0${days}`
